...
setwd("C:/Personal/Psychology/Projects/DDE/git/Game/Data/absolute/v1/Halfway 3");
...
colnames(data) <- c("MFonMB","Choice","Subj","Condition");

# normal crit trials
data_crits = data[coeflist_abs>=0,];
...
colnames(data_models) <- c("Reward","Choice","Subj","Condition");

# Test all absolute trials
# Do people use the absolute value data?
data_models_crits = data_models[data_models$Condition == 1,];
model1 = glmer(Choice~Reward+abs(Reward)+(1|Subj)+(0+Reward+abs(Reward)|Subj),family=binomial,data=data_models_crits);

coeflist_abs=coef(model1)$Subj$Reward;
