...
for thisSubj = 1:numSubjects
    ...
    for thisRound = index(2:end)
        ...
        %if S2(thisRound-1) ~= 5 || Crit(thisRound)==1 || ~any(Action(thisRound-1)==[Opt1(thisRound) Opt2(thisRound)]), keep(thisRound) = false;
        %if S2(thisRound-1)~=5 || Crit(thisRound)==1, keep(thisRound) = false;
        %if Crit(thisRound)==1, keep(thisRound) = false;
        if S2(thisRound-1)~=5 || Crit(thisRound)==1, keep(thisRound) = false;
        else
            reward(thisRound) = Re(thisRound-1);
            subjIDs(thisRound) = id(thisRound);
...
csvwrite('Parsed_models_abs.csv',[reward(keep_models) choices(keep_models) subjIDs(keep_models) condition(keep_models)]);
