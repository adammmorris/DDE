...
%for i=1:numSubj, condition(id==i) = instructManip(i); end
...
choices2 = zeros(numDataPoints,1); % 1 is corresponding action, 0 is not
keep_stage2 = false(numDataPoints,1);
...
for thisSubj = 1:numSubjects
    ...
    if numTrialsCompleted(subjID) > minNumTrials && numTrialsCompleted(subjID) <= maxNumTrials && finalScores(subjID) > scoreCutoff
        for thisRound = index(4:end)
            if Crit(thisRound)==1 && (Re(thisRound-2) > 2 && Action2(thisRound-3) == Action2(thisRound-2) && S2(thisRound-3)~=5 && S2(thisRound-2)~=5)
                keep(thisRound) = true;
                ...
                if choices(thisRound) == 1
                    keep_stage2(thisRound) = true;
                    choices2(thisRound) = Action2(thisRound)==Action2(thisRound-2);
                end
            end
        end
    end
...
csvwrite('Parsed_ttests2.csv',[MFonMB(keep_stage2) choices(keep_stage2) choices2(keep_stage2) subjIDs(keep_stage2) condition(keep_stage2)]);

%% Write models
% For the models, we need to drop people who made the same choice
%   every critical trial
keep_models = keep_stage2;
subjIDs_unique = unique(id);
for i = 1:numSubjects
    if length(unique(choices2(keep_stage2 & id==subjIDs_unique(i))))==1, keep_models(id==subjIDs_unique(i)) = false; end
end

% Grand mean center
MFonMB(keep_models) = MFonMB(keep_models) - mean(MFonMB(keep_models));

csvwrite('Parsed_models2.csv',[MFonMB(keep_models) choices(keep_models) choices2(keep_models) subjIDs(keep_models) condition(keep_models)]);
