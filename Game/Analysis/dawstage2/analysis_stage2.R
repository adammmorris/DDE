...
setwd("C:/Personal/Psychology/Projects/DDE/git/Game/Data/dawstage2/Real (v2)");
...
## T-TESTS
data = read.csv("Parsed_ttests2.csv",header=FALSE);
colnames(data) <- c("MFonMB","Choice","Choice2","Subj","Condition");
...
for (i in 1:numSubj) {
	...
	re_lo[i] = mean(data_crits[data_crits$Subj==subj,]$Choice2[data_crits[data_crits$Subj==subj,]$MFonMB<0]);
	re_hi[i] = mean(data_crits[data_crits$Subj==subj,]$Choice2[data_crits[data_crits$Subj==subj,]$MFonMB>0]);
}
...
## MODELS
data_models = read.csv("Parsed_models2.csv",header=FALSE);
colnames(data_models) <- c("MB","MF","MFonMB","Choice","Subj","Condition");
...
# incongruent crit trials - for 2 trial type games only
...
# interaction - for 2 trial type games only
...
# These are for 2 trial type games only
