...
for thisSubj = 1:numSubjects
    ...
    if numTrialsCompleted(subjID) > minNumTrials && numTrialsCompleted(subjID) <= maxNumTrials && finalScores(subjID) > scoreCutoff
        for thisRound = index
            if round1(thisRound) > practiceCutoff && Action(thisRound-2) ~= -1 && Action2(thisRound-2) == 1 %&& (novel(OptNum(thisRound)) && novel(NUMBERS(Action(thisRound-1)+1)-OptNum(thisRound)))
                keep(thisRound) = true;
