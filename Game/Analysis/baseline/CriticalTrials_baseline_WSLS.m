...
for thisSubj = 1:numSubjects
    ...
    if numTrialsCompleted(subjID) > minNumTrials && numTrialsCompleted(subjID) <= maxNumTrials && finalScores(subjID) > scoreCutoff
        for thisRound = index
            if round1(thisRound) > practiceCutoff && S2(thisRound-1) == 6 && ~any(chosenAction == [Opt1(thisRound) Opt2(thisRound)]) && any(chosenAction_cor == [Opt1(thisRound) Opt2(thisRound)]) && thisRound < index(end)
