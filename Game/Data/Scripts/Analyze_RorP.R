...
re_mid <- vector(mode='integer',length=numSubj);
...
for (i in 1:numSubj) {
	...
	re_mid[i] = mean(data_crits[data_crits$Subj==subj,]$Choice[data_crits[data_crits$Subj==subj,]$MFonMB==0]);
}
...
data_models$RorP <- (data_models$MFonMB>=0)+0;
#data_models$MFonMB[data_models$MFonMB<0] <- data_models$MFonMB[data_models$MFonMB<0]*-1;

data_crits_models = data_models[data_models$Crits==1,];
model = glmer(Choice~MFonMB*RorP+(1|Subj)+(0+MFonMB*RorP|Subj),family=binomial,data=data_crits_models);
