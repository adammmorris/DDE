...
for (i in 1:numSubj) {
	re_lo[i] = mean(data_crits[data_crits$Subj==i,]$Choice[data_crits[data_crits$Subj==i,]$MFonMB<0]);
	re_hi[i] = mean(data_crits[data_crits$Subj==i,]$Choice[data_crits[data_crits$Subj==i,]$MFonMB>0]);
}
