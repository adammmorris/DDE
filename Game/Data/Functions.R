## stuff for within-subject error bars
...
doBarPlot <- function(data.raw,title,fileName) {
require(ggplot2);
...
data.summary.within <- summarySEwithin(data.raw, measurevar="value", withinvars="trials",
                        idvar="subj", na.rm=FALSE, conf.interval=.95);
data.summary$me <- data.summary.within$ci;

png(fileName) # Write to PNG
print(ggplot(data.summary, aes(x = trials, y = mean.choice)) + 
