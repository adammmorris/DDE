...
path = 'C:\Personal\School\Brown\Psychology\DDE Project\git\Model\board4_1tt.mat';
...
numFeatureValues = 3;
numTrialTypes = 2;
numTrialTypes_using = 1; % SET THIS TO 2 IF YOU WANT TO ACTUALLY USE BOTH TRIAL TYPES
numGoals = numFeatureValues * numTrialTypes;
numStates = 6; % remember to preserve the state # integrity

% Trial types
% numTotalRounds x numAgents
if numTrialTypes_using==2, trialTypes = round(rand(numTotalRounds,numAgents)+1);
else trialTypes = 2*ones(numTotalRounds,numAgents); % NOTE: if we do a single trial type, it's "color" (meaning actions 1 and 3 both lead to blue, and actions 2 and 4 both lead to red)
end
...
%% Save
save(path,'orig_simulation','numCrits','numOptions','numFeatureValues','numTrialTypes','numTrialTypes_using','numStates','numGoals','trialTypes','options','transitions','rewards','rewardRange_hi','rewardRange_lo','criticalTrials','features','goals');