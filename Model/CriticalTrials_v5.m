...
c_bysubj_lo = zeros(numSubjects,1);
c_bysubj_hi = zeros(numSubjects,1);
...
for thisSubj = 1:numSubjects
            ...
            if round1(thisRound) <= practiceCutoff
                ...
                if MFonMB_alltrials == 0
                    ...
                    while found == 0 && (thisRound - counter) >= index(1) && counter <= distance_cutoff
                        ...
                        %   optY=optX_cor)
                        if ~any(receivedGoal==[chosenAction chosenAction_cor]) && chosenAction~=5 && optX==chosenAction_cor && (halfCrits==0 || ~any(receivedGoal==[optY optY_cor])) && optY~=optX_cor
                            ...
                            if Type(thisRound-counter)==trialType
                                ...
                                if optX==chosenAction_cor, MFonMB_X(thisRound) = Re(thisRound-counter);
                                else MFonMB_Y(thisRound) = Re(thisRound-counter);
                                end
                            end
                        else
                            counter = counter + 1;
    ...
    logindex = false(numDataPoints,1);
    logindex(index)=true;
    c_bysubj_lo(thisSubj) = mean(choices(logindex&MFonMB_X<prctile(MFonMB_X,10)));
    c_bysubj_hi(thisSubj) = mean(choices(logindex&MFonMB_X>prctile(MFonMB_X,90)));
end
...
c_bysubj_lo = removerows(c_bysubj_lo,'ind',tosslist);
c_bysubj_hi = removerows(c_bysubj_hi,'ind',tosslist);
...
MFonMB_X_dummy = MFonMB_X>prctile(MFonMB_X,90);
toremove = (MFonMB_X>prctile(MFonMB_X,10))&(MFonMB_X<prctile(MFonMB_X,90));
MFonMB_X_dummy = removerows(MFonMB_X_dummy,'ind',toremove);
choices_dummy = removerows(choices,'ind',toremove);
subj_dummy = removerows(subjIDs,'ind',toremove);
critTrials_dummy = removerows(critTrials,'ind',toremove);
...
%csvwrite('test.csv',[MFonMB_X(critTrials) subjIDs(critTrials) choices(critTrials)]);csvwrite('test.csv',[MFonMB_X(critTrials_incog) subjIDs(critTrials_incog) choices(critTrials_incog)]);