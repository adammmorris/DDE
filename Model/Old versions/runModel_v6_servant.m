...
% weights should be [modelBased goalLearner]
...
function [earnings, negLL, results] = runModel_v6_servant(agent_params, weights, numRounds, numAgents)
...
weight_smartModelFree = ones(numAgents,1) - weight_modelBased;
...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        ...
        % Now we're in the big leagues - the real rounds
            ...
            %   modelBased?
            Q_modelBased(1,action_options) = (transition_probs(action_options,:) * Q_modelBased(goals(trialType,features(:,trialType))+1,1))' +  + weight_goalLearner(thisAgent) * Q_goalLearner(1,goal_options);
            
            %% Get weighted Q
            Q_weighted = weight_modelBased(thisAgent) * Q_modelBased(1,action_options)' + weight_smartModelFree(thisAgent) * Q_smartModelFree(1,action_options,trialType)';
