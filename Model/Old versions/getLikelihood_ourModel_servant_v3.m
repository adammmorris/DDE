...
%% Inputs:
% params should be the 4x1 vector [lr,beta,mb_weight,goalLearner_weight]
...
function [negLL] = getLikelihood_ourModel_servant_v3(params,numRounds,ourTrialTypes,ourOption1,ourOption2,ourChoices,ourState2,ourRewards)
...
for thisRound = 1:numTotalRounds
    ...
    if (thisRound <= numPracticeRounds)
        ...
        %   modelBased?
        Q_modelBased(1,action_options) = (transition_probs(action_options,:) * Q_modelBased(goals(trialType,features(:,trialType))+1,1))' +  + weight_goalLearner * Q_goalLearner(1,goal_options);
