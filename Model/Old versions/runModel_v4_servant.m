...
% - Added servant vs. equal distinction
...
function [avgEarnings, stdEarnings, negLL] = runModel_v4_servant(global_params, weights, numRounds, numAgents)
...
weight_goalLearner = weights(4); % this is a servant, so it's a separate weight
...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        ...
        % Now we're in the big leagues - the real rounds
            ...
            %   modelBased?
            Q_modelBased(1,action_options) = (transition_probs(action_options,:) * Q_modelBased(goals(trialType,features(:,trialType)),1))' + weight_goalLearner * Q_goalLearner(1,goal_options);
            
            %% Get weighted Q
            Q_weighted = weight_modelBased * Q_modelBased(1,action_options)' + weight_smartModelFree * Q_smartModelFree(1,action_options,trialType)' + weight_dumbModelFree * Q_dumbModelFree(1,action_options)';
            ...
            %% Make choice
            % Take out any infinities
            softmax = exp(beta*Q_weighted);
            softmax(isinf(softmax)) = realmax;
            probs = softmax / sum(softmax);
            ...
            %% Update goal learner
            % It should only update on trials in which there was truly a goal
            %   selected - when the model-based's top choice won
