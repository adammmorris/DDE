        ...
        for thisRound = index
            if round1(thisRound) <= practiceCutoff
                ...
                while found == 0 && (thisRound - counter) >= index(1)
                    ...
                    %   optX_cor
                    if trialType == Type(thisRound-counter) && any((S2(thisRound-counter)-1) == [optX optX_cor optY optY_cor])
                        % Woohoo!
                        if any((S2(thisRound-counter)-1) == [optX optX_cor])
                            MB(thisRound) = -Re(thisRound-counter);
                        else
                            MB(thisRound) = Re(thisRound-counter);
                        end
                        found = 1;
                ...
                while found == 0 && (thisRound - counter) >= index(1)
                    ...
                    if any(Action(thisRound-counter) == [optX optY])
                        % Woohoo!
                        if Action(thisRound-counter)==optX
                            MF(thisRound) = -Re(thisRound-counter);
                        else
                            MF(thisRound) = Re(thisRound-counter);
                        end
                        found = 1;
                ...
                while found == 0 && (thisRound - counter) >= index(1)
                    % In this round, did the subject choose optX?
                    if Type(thisRound-counter) == trialType && any(Action(thisRound-counter) == [optX optY])
                        % Woohoo!
                        if Action(thisRound-counter)==optX
                            SMF(thisRound) = -Re(thisRound-counter);
                        else
                            SMF(thisRound) = Re(thisRound-counter);
                        end
                        found = 1;
                ...
                if MFonMB_alltrials == 1
                    ...
                    while found == 0 && (thisRound - counter) >= index(1)
                        ...
                        if Type(thisRound-counter) == trialType && any(Action(thisRound-counter) == [optX optX_cor])
                            % Woohoo!
                            if any(Action(thisRound-counter)==[optX optX_cor])
...
% Residuals
if residuals == 1
    MF = MF - MB; % I think we have to make these residuals also?  B/c otherwise which would MFonMB be a residual in reference to?
    MF = MF - MB;
    SMF = SMF-MB;
    MFonMB=MFonMB-MB;
...
SMF = removerows(SMF,'ind',rowsToToss);
...
%csvwrite('test.csv',[MB MF SMF MFonMB subjIDs choices]);