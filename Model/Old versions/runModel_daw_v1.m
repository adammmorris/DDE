...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        ...
        [~,likely_state_options(1)] = max(transition_probs(curActions(1),:),[],2); % what will these likely lead to?
        [~,likely_state_options(2)] = max(transition_probs(curActions(2),:),[],2);
        ...
        % Now we're in the big leagues - the real rounds
            ...
            Q_options(state1,:) = w_interMF*Q_options(state1,:)+(1-w_interMF)*max(Q_options(2:3,:),[],2);
