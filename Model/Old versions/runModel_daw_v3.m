...
function [earnings, negLL, results] = runModel_daw_v3(params, boardPath)
...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        ...
        % Now we're in the big leagues - the real rounds
            ...
            % Get weighted Q
            Q_weighted = w_MF*Q_MF(state1,curActions) + w_MB*Q_MB(state1,curActions) + (1-w_MF-w_MB)*Q_MFonMB_options(state1,:)*Q_MFonMB_actions(:,curActions);

            a_MFG = randsample(numOptions,1,true,
            ...
            % Get weighted Q
            Q_weighted = w_MF*Q_MF(state2,S2_actions) + w_MB*Q_MB(state2,S2_actions) + (1-w_MF-w_MB)*Q_MFonMB_options(state2,S2_actions);
