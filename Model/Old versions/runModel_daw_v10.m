...
function [earnings, results] = runModel_daw_v10(params)
...
% Generate correct transition matrix
...
% Set agent's transition matrix to correct one
transition_probs = transition_probs0;

% Outputs
earnings = zeros(numAgents,1);
results = zeros(numAgents*numTotalRounds,9);
...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        crit = any(criticalTrials==thisRound); % Are we in a critical trial? (The critical trial numbers are hardcoded in buildBoard.m)
        ...
        if crit
            ...
            % Otherwise, use what board_1b.mat gives us
            curActions = availableActions(thisRound,:,thisAgent);
        end
                
        % Update MB models (see SI)
        % S2_states, S2_actions, etc. are set in buildBoard.m
