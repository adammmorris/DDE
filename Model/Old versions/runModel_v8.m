...
function [earnings, negLL, results] = runModel_v8(agent_params, weights, numRounds, numAgents, servant, boardName, magicBoard, numCriticalTrials)
...
distance_cutoff = 3;
criticalTrials = zeros(numCriticalTrials,1);

for i = 1:numCriticalTrials
    criticalTrials(i) = randsample(templist(good),1);
...
for thisAgent = 1:numAgents
    ...
    for thisRound = 1:numTotalRounds
        ...
        if any(criticalTrials==(thisRound-1))
            % If we are, force options & trial type
            action_options = [getCorrespondingAction(prevChoice,prevType,orig_simulation) 5];
            trialType = prevType;
        elseif any(criticalTrials==thisRound) % Are we in a critical trial?
            % If so, don't allow 5
        ...
        % Now we're in the big leagues - the real rounds
            ...
            reward = rewards(magic,thisRound,trialType,newstate_feature);
